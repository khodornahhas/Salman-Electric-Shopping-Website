...
<div class="w-full px-6 max-w-7xl mx-auto py-6">
    <div class="mb-8">
        ...
        <form method="GET" action="{{ route('admin.products') }}" id="filtersForm"
            class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-white p-4 rounded-lg shadow-sm border border-gray-100">

            <div class="flex flex-col sm:flex-row flex-wrap items-center gap-3 w-full">
                <div class="flex items-center bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 w-full sm:w-64">
                ...
                <select name="price_sort" id="filter-price" 
                    class="border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-700 focus:ring-2 focus:ring-blue-500 focus:outline-none">
                    <option value="">Sort by Price</option>
                    <option value="low_high" {{ request('price_sort') == 'low_high' ? 'selected' : '' }}>Low to High</option>
                    <option value="high_low" {{ request('price_sort') == 'high_low' ? 'selected' : '' }}>High to Low</option>
                </select>
            </div>

            <a href="{{ route('admin.products.create') }}"
                class="bg-blue-600 text-white px-5 py-2 rounded-lg hover:bg-blue-700 transition shadow-sm text-center whitespace-nowrap">
                + Add Product
            </a>
        </form>
    </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#filter-brand, #filter-category, #filter-price').on('change', function() {
            fetchProducts();
        });
        ...
        let price_sort = $('#filter-price').val();
