        ...
        <div class="flex justify-between flex-1 sm:hidden">
            ...
            @else
                <a href="{{ $paginator->previousPageUrl() }}"
                   class="relative inline-flex items-center px-4 py-2 text-sm font-medium text-gray-800 bg-white border border-gray-300 leading-5 rounded-md hover:bg-gray-100 focus:outline-none focus:ring ring-gray-300 focus:border-gray-400 active:bg-gray-200 transition">
                    {!! __('pagination.previous') !!}
                </a>
            @endif
            ...
            @if ($paginator->hasMorePages())
                <a href="{{ $paginator->nextPageUrl() }}"
                   class="relative inline-flex items-center px-4 py-2 ml-3 text-sm font-medium text-gray-800 bg-white border border-gray-300 leading-5 rounded-md hover:bg-gray-100 focus:outline-none focus:ring ring-gray-300 focus:border-gray-400 active:bg-gray-200 transition">
                    {!! __('pagination.next') !!}
                </a>
            @else
                ...
                <span class="relative z-0 inline-flex rtl:flex-row-reverse shadow-sm rounded-md">
                    ...
                    @else
                        <a href="{{ $paginator->previousPageUrl() }}" rel="prev"
                           class="relative inline-flex items-center px-2 py-2 text-sm font-medium text-gray-600 bg-white border border-gray-300 rounded-l-md leading-5 hover:text-gray-800 hover:bg-gray-100 focus:z-10 focus:outline-none focus:ring ring-gray-300 focus:border-gray-400 active:bg-gray-100 transition ease-in-out duration-150"
                            ...
                            @foreach ($element as $page => $url)
                                @if ($page == $paginator->currentPage())
                                    <span aria-current="page">
                                        <span class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-semibold text-gray-700 bg-gray-200 border border-gray-300 cursor-default leading-5">
                                            {{ $page }}
                                        </span>
                                    </span>
                                @else
